...
-- -----------------------------------------------------
-- Table `bootcamp`.`extendidos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bootcamp`.`extendidos` (
  `idextendido` INT(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`idextendido`),
  INDEX `fk_extendido_clima_idx` (`idclima` ASC),
  CONSTRAINT `fk_extendido_clima`
    FOREIGN KEY (`idclima`)
    REFERENCES `bootcamp`.`climas` (`idclima`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 19
